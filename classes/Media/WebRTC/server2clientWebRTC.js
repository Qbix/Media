...
module.exports = function(socket, io, rtmpUrl) {
    ...
    async function initPeerConnection(participant, connectionManager) {
        ...
        try {
            ...
            webrtcNamespace.to(nspName + '#' + participant.id).emit('Media/webrtc/recording', {initiatorId: socket.client.id, connection:connection});
    ...
    socket.on('Media/webrtc/recording', function(message) {
       ...
       /* io.of('/webrtc').in(socket.webrtcRoom.id).allSockets().then(function (clients) {
    ...
    socket.on('Media/webrtc/roomSwitched', function(message) {
    ...
    socket.on('Media/webrtc/joined', function (identity) {
                ...
                async function asyncCall(connectionManager, connectionInfo) {
                    ...
                    try {
                        await connection.applyOffer(message).then(function (answer) {
                            ...
                            socket.emit('Media/webrtc/signalling', answer);
                        });

                        //res.send(connection.toJSON().remoteDescription);
                        //socket.to(nspName + '#' + socket.client.id).emit('Media/webrtc/signalling', message);
        ...
        function drawVideosOnCanvas() {
            ...
            /*setInterval(function () {
                ...
                io.of('/webrtc').in(socket.webrtcRoom.id).allSockets().then(function (clients) {
                    //if(_debug) console.log(clients);
                    for (var i in clients) {
                        //console.log('Media/webrtc/serverVideo', clients[i]);
                        webrtcNamespace.to(clients[i]).emit('Media/webrtc/serverVideo',  {size: {width: imgData.width, height: imgData.height}, data: imgData.data.buffer});
                    }
                });
    ...
    function beforeOffer(peerConnection, participant) {
        ...
        console.log('Media/webrtc/serverVideo start 0');
