...
module.exports = function (socket, io) {
    ...
    if(WebRTC.roomsWithLimits == null) {
        WebRTC.roomsWithLimits = [];
    }
    ...
    WebRTC.Room = function (id) {
        ...
        this.remove = function () {
            ...
            var room = this;
            for (var i = WebRTC.roomsWithLimits.length - 1; i >= 0; i--) {
                if (WebRTC.roomsWithLimits[i] == room) {
                    WebRTC.roomsWithLimits.splice(i, 1);
                    break;
                }
            }
            room = null;
    ...
    function init(socketInstace, ioInstance, roomName) {
        let socket = socketInstace;
        io = ioInstance;
        return new Promise(function (resolve, reject) {
            let roomExists = false;
            log('roomManager: joined', socket.adapter.rooms);
            log('roomManager: joined 2', WebRTC.roomsWithLimits.length);
            log('roomManager: joined roomName)', roomName);
            ...
            if (!roomExists) {
                log('roomManager: Room not exists', WebRTC.roomsWithLimits.length);
    
                for (let r in WebRTC.roomsWithLimits) {
                    log('roomManager: Room not exists for', WebRTC.roomsWithLimits[r].id);
    
                    if (roomName == WebRTC.roomsWithLimits[r].id) {
                        socket.webrtcRoom = WebRTC.roomsWithLimits[r];
                        if(!socket.webrtcRoom.roomStream) {
                            loadRoomStream().then(function (){
                                onInited();
                            });
                        } else {
                            onInited();
                ...
                log('roomManager: Room exists', WebRTC.roomsWithLimits.length);
            ...
            if (!socket.webrtcRoom) {
                log('roomManager: Room not exists 2: create', roomName);
    
                var currentRoom = new WebRTC.Room(roomName);
                currentRoom.roomPublisherId = socket.roomPublisherId;
                socket.webrtcRoom = currentRoom;
                WebRTC.roomsWithLimits.push(currentRoom);
    
                
                log('roomManager: Room not exists 2: create 2');
    
                if(!currentRoom.roomStream) {
                    loadRoomStream().then(function () {
                        onInited();
                    });
                }
            ...
            function loadRoomStream() {
                return new Promise(function(resolve) {
                    let userPlatformId = socket.username.split('\t')[0];
                    Q.plugins.Streams.fetchOne(userPlatformId, socket.webrtcRoom.roomPublisherId, 'Media/webrtc/' + socket.roomId, function (err, stream) {
                        if (err || !stream) {
                            console.log(err);
                            return;
                        }
                        socket.webrtcRoom.roomStream = stream;
                        ...
                        if (_globalWebRTCOptions['limits'] != null) {
                            ...
                            socket.webrtcRoom.limits = _globalWebRTCOptions['limits'];
                        } else {
                            socket.webrtcRoom.limits = {};
                        }
                        ...
                        if (limitsConfig != null) {
                            ...
                            if (limitsConfig['video'] != null) {
                                socket.webrtcRoom.limits.video = limitsConfig['video'];
                            }
                            if (limitsConfig['audio'] != null) {
                                socket.webrtcRoom.limits.audio = limitsConfig['audio'];
                            }
                            if (limitsConfig['minimalTimeOfUsingSlot'] != null) {
                                socket.webrtcRoom.limits.minimalTimeOfUsingSlot = limitsConfig['minimalTimeOfUsingSlot'];
                            }
                            if (limitsConfig['timeBeforeForceUserToDisconnect'] != null) {
                                socket.webrtcRoom.limits.timeBeforeForceUserToDisconnect = limitsConfig['timeBeforeForceUserToDisconnect'];
                            }
                        }
                        log('roomManager: currentRoom.limits', socket.webrtcRoom.limits);
            ...
            function onInited() {
                ...
                for (let r in socket.webrtcRoom.participants) {
                    if (socket.id == socket.webrtcRoom.participants[r].id) {
                ...
                if (socket.webrtcParticipant == null) {
                    var currentParticipant = new WebRTC.Participant(socket.id)
                    currentParticipant.sid = socket.socketSid;
                    currentParticipant.username = socket.username;
                    currentParticipant.connectedTime = socket.username.split('\t')[1];
                    currentParticipant.userPlatformId = socket.username.split('\t')[0];
                ...
                connectUserToLimitsRoom(socket);
                ...
                log('roomManager: LIMITS', socket.webrtcRoom);

                resolve(socket.webrtcRoom.limits);
            }
            
        });
    }

    function connectUserToLimitsRoom(socketConnection) {
        let socket = socketConnection;
        declareEventsHandlers();
        function declareEventsHandlers() {
            socket.on('Media/webrtc/controlsLoaded', onControlsLoaded);
            function onControlsLoaded() {
                ...
                for (let r in socket.webrtcRoom.cameraRequestsQueue) {
                    ...
                    socket.emit('Media/webrtc/requestCamera', emitMsg);
                }
                ...
                for (let r in socket.webrtcRoom.micRequestsQueue) {
                    ...
                    socket.emit('Media/webrtc/requestMic', emitMsg);
                }
            }
    
            socket.on('Media/webrtc/getLimitsInfo', getLimitsInfo);
            function getLimitsInfo() {
                log('getLimitsInfo: START')
                ...
                socket.emit('Media/webrtc/getLimitsInfo', emitMsg);
            }
    
            socket.on('Media/webrtc/canITurnCameraAndMicOn', canITurnCameraAndMicOn);
            function canITurnCameraAndMicOn() {
                ...
                if (audioSlotIsAvailable && socket.webrtcRoom.micRequestsQueue.length == 0 && videoSlotIsAvailable && socket.webrtcRoom.cameraRequestsQueue.length == 0) {
                    ...
                    socket.emit('Media/webrtc/canITurnCameraAndMicOn', { type: 'answer', answerValue: true, limit: socket.webrtcRoom.limits.audio });
            ...
            socket.on('Media/webrtc/canITurnCameraOn', canITurnCameraOn);
            function canITurnCameraOn() {
                ...
                if (socket.webrtcParticipant.videoTracks().length != 0) {
                    ...
                    socket.emit('Media/webrtc/canITurnCameraOn', { type: 'answer', answerValue: true, limit: socket.webrtcRoom.limits.video });
            ...
            socket.on('Media/webrtc/canITurnMicOn', canITurnMicOn);
            function canITurnMicOn() {
                ...
                if (socket.webrtcParticipant.videoTracks(true).length == 0) {
                    ...
                    if (audioSlotIsAvailable && socket.webrtcRoom.micRequestsQueue.length == 0 && videoSlotIsAvailable && socket.webrtcRoom.cameraRequestsQueue.length == 0) {
                        ...
                        socket.emit('Media/webrtc/canITurnMicOn', { type: 'answer', answerValue: true, limit: socket.webrtcRoom.limits.audio });
                    } else if ((videoSlotIsAvailable && socket.webrtcRoom.cameraRequestsQueue.length == 0) && (!audioSlotIsAvailable || socket.webrtcRoom.micRequestsQueue.length != 0)) {
            ...
            socket.on('Media/webrtc/cameraDisabled', onCameraDisabled);
            function onCameraDisabled() {
                if (!socket.webrtcRoom) return;
                log('cameraDisabled START', socket.webrtcParticipant.id)
    
                //inform all users that user turned his camera off
                socket.to(socket.webrtcRoom.id).emit('Media/webrtc/cameraDisabled', {
            ...
            function onMicDisabled() {
                ...
                } else if(socket.webrtcParticipant.cameraRequestCountdown.queueItem && socket.webrtcParticipant.cameraRequestCountdown.queueItem.requestType != null) {
            ...
            socket.on('Media/webrtc/cameraEnabled', onCameraEnabled);
            function onCameraEnabled() {
                if (!socket.webrtcRoom) return;
                log('cameraEnabled')
    
                socket.to(socket.webrtcRoom.id).emit('Media/webrtc/cameraEnabled', {
                    fromSid: socket.webrtcParticipant.id,
                });
            ...
            socket.on('Media/webrtc/micEnabled', onMicEnabled);
            function onMicEnabled() {
                if (!socket.webrtcRoom) return;
                log('micEnabled')
    
                socket.to(socket.webrtcRoom.id).emit('Media/webrtc/micEnabled', {
                    fromSid: socket.webrtcParticipant.id,
                });
            ...
            socket.on('Media/webrtc/cameraRequestCanceled', onCameraRequestCanceled);
            function onCameraRequestCanceled() {
                if (!socket.webrtcRoom) return;
                ...
                for (let p in participants) {
                    ...
                    if (nextUserInQueue) {
                        ...
                        socket.to(participants[p].id).emit('Media/webrtc/cancelForceTurnCameraOffTimer');
                ...
                socket.broadcast.to(socket.webrtcRoom.id).emit('Media/webrtc/cameraRequestCanceled', {
                    fromSid: socket.webrtcParticipant.id,
                });
                socket.emit('Media/webrtc/cameraRequestCanceled', {
                    fromSid: socket.webrtcParticipant.id,
                });
            }
    
            /* socket.on('Media/webrtc/participantDisconnected', function () {
            ...
            function onDisconnect() {
                log('disconnect');
                if (!socket.webrtcRoom) return;
                if (socket.webrtcParticipant.id == socket.id) {
                    log('disconnect socket.webrtcParticipant.id != socket.id', socket.webrtcParticipant.id, socket.id)
                ...
                io.of('/webrtc').in(socket.webrtcRoom.id).fetchSockets().then(function (clients) {
                    log('disconnect: before close room', clients.length)
                    if (clients.length == 0) socket.webrtcRoom.close();
                });
            }
    
            socket.unsubscribeLimitsHandlers = function () {
                socket.removeListener('Media/webrtc/controlsLoaded', onControlsLoaded);
                socket.removeListener('Media/webrtc/getLimitsInfo', getLimitsInfo);
                socket.removeListener('Media/webrtc/canITurnCameraAndMicOn', canITurnCameraAndMicOn);
                socket.removeListener('Media/webrtc/canITurnCameraOn', canITurnCameraOn);
                socket.removeListener('Media/webrtc/canITurnMicOn', canITurnMicOn);
                socket.removeListener('Media/webrtc/cameraDisabled', onCameraDisabled);
                socket.removeListener('Media/webrtc/micDisabled', onMicDisabled);
                socket.removeListener('Media/webrtc/cameraEnabled', onCameraEnabled);
                socket.removeListener('Media/webrtc/micEnabled', onMicEnabled);
                socket.removeListener('Media/webrtc/cameraRequestCanceled', onCameraRequestCanceled);
        ...
        function addMeToCameraQueue(options) {
            ...
            if (userWhoUsesCameraMostOfAll) {
                ...
                if (mostNewVideoTrack.kind == 'videoavatar') {
                    ...
                    userWhoUsesCameraMostOfAll.micRequestCountdown.timer = userWhoUsesCameraMostOfAll.cameraRequestCountdown.timer = setTimeout(function () {
                        log('addMeToCameraQueue: force disable mic+avatar to make avatar slot available', userWhoUsesCameraMostOfAll.id)
                        socket.to(userWhoUsesCameraMostOfAll.id).emit('Media/webrtc/forceTurnMicOff');
                    }, waitingTime);
                    ...
                    userWhoUsesCameraMostOfAll.cameraRequestCountdown.timer = setTimeout(function () {
                        ...
                        socket.to(userWhoUsesCameraMostOfAll.id).emit('Media/webrtc/forceTurnCameraOff');
                    }, waitingTime);
            ...
            socket.broadcast.to(socket.webrtcRoom.id).emit('Media/webrtc/requestCamera', emitMsg);
            socket.emit('Media/webrtc/requestCamera', emitMsg);
            return queueItem;
        }
        ...
        function isVideoSlotAvailable() {
            ...
            for (let r in participants) {
                ...
                //if(socket.id == participant.id) continue;
        ...
        function addMeToMicQueue(options) {
            ...
            if (userWhoUsesMicMostOfAll) {
                ...
                log('addMeToMicQueue: waitingTime', waitingTime, socket.webrtcRoom.limits)
                log('addMeToMicQueue: timeUserUsesMic', timeUserUsesMic, mostNewAudioTrack.createdTime)
                ...
                if(userWhoUsesMicMostOfAll.micRequestCountdown.timer == null) {
                    userWhoUsesMicMostOfAll.micRequestCountdown.timer = setTimeout(function () {
                        ...
                        socket.to(userWhoUsesMicMostOfAll.id).emit('Media/webrtc/forceTurnMicOff');
                    }, waitingTime);
                }
            ...
            socket.broadcast.to(socket.webrtcRoom.id).emit('Media/webrtc/requestMic', emitMsg);
            socket.emit('Media/webrtc/requestMic', emitMsg);
        ...
        function giveVideoSlotToNextUser() {
            ...
            if (socket.webrtcParticipant.cameraRequestCountdown.queueItem) {
                ...
                if(!queueItem) { //probably user disconnected
                    return
                }
                ...
                let reserveThisAudioSlotForNextUserInQueue = function (nextItemInMicQueue, reserveOnlyVideoSlot) {
                    ...
                    userWhoJustGotAudioSlot.micRequestCountdown.timer = setTimeout(function () {
                        ...
                        socket.to(userWhoJustGotAudioSlot.id).emit('Media/webrtc/forceTurnMicOff');
                ...
                let reserveThisVideoSlotForNextUserInQueue = function (nextItemInCameraQueue, withoutTimer) {
                    ...
                    if(!withoutTimer) {
                        userWhoJustGotVideoSlot.cameraRequestCountdown.timer = setTimeout(function () {
                            ...
                            socket.to(userWhoJustGotVideoSlot.id).emit('Media/webrtc/forceTurnCameraOff');
                        }, socket.webrtcRoom.limits.minimalTimeOfUsingSlot);
                    }
                ...
                if (queueItem.requestType == 'canITurnMicOn') {
                    ...
                    socket.to(giveCameraTo.id).emit('Media/webrtc/canITurnMicOn', { type: 'answer', answerValue: true, timeProvided: (nextAudioRequestInQueue || nextVideoRequestInQueue) ? socket.webrtcRoom.limits.minimalTimeOfUsingSlot : null, limit: socket.webrtcRoom.limits.audio });
                    ...
                    //if there is one more (next) user in queue, set limit provided time for current user who got slot via sending timeProvided option, it will show countdown timer on client side
                    socket.to(giveCameraTo.id).emit('Media/webrtc/' + queueItem.requestType, { type: 'answer', answerValue: true, timeProvided: nextVideoRequestInQueue ? socket.webrtcRoom.limits.minimalTimeOfUsingSlot : null, limit: socket.webrtcRoom.limits.video });
                ...
                socket.emit('Media/webrtc/cancelForceTurnCameraOffTimer');
                ...
                let reserveThisVideoSlotForNextUserInQueue = function (nextItemInCameraQueue) {
                    ...
                    userWhoJustGotVideoStream.participant.cameraRequestCountdown.timer = setTimeout(function () {
                        ...
                        socket.to(userWhoJustGotVideoStream.participant.id).emit('Media/webrtc/forceTurnCameraOff');
                    }, socket.webrtcRoom.limits.minimalTimeOfUsingSlot);
                ...
                if (!firstUserInQueue.participant.pendingAudioSlotRequest()) {
                    ...
                    socket.to(firstUserInQueue.participant.id).emit('Media/webrtc/' + firstUserInQueue.requestType, { answerValue: true, limit: socket.webrtcRoom.limits.video });
                ...
                if (firstUserInQueue.userThatWillGiveSlot) {
                    ...
                    if (nextUserInQueue) {
                        ...
                        socket.to(firstUserInQueue.userThatWillGiveSlot.id).emit('Media/webrtc/cancelForceTurnCameraOffTimer');
        ...
        function giveVideoAndAudioSlotToRequestor(userWhoRequestedAudioAndVideoSlots) {
            ...
            let reserveThisAudioSlotForNextUserInQueue = function(nextItemInMicQueue) {
                ...
                userWhoJustGotAudioAndVideoSlots.micRequestCountdown.timer = setTimeout(function () {
                    ...
                    socket.to(userWhoJustGotAudioAndVideoSlots.id).emit('Media/webrtc/forceTurnMicOff');
                }, socket.webrtcRoom.limits.minimalTimeOfUsingSlot);
            ...
            let reserveThisVideoSlotForNextUserInQueue = function(nextItemInCameraQueue, withoutTimer) {
                ...
                if (!withoutTimer) {
                    userWhoJustGotAudioAndVideoSlots.cameraRequestCountdown.timer = setTimeout(function () {
                        ...
                        socket.to(userWhoJustGotAudioAndVideoSlots.id).emit('Media/webrtc/forceTurnCameraOff');
                    }, socket.webrtcRoom.limits.minimalTimeOfUsingSlot);
                }
            ...
            if (requestType == 'canITurnMicOn') {
                ...
                socket.to(userWhoRequestedAudioAndVideoSlots.id).emit('Media/webrtc/canITurnMicOn', {
                    type: 'answer',
                    answerValue: true,
                ...
                userWhoRequestedAudioAndVideoSlots.tracks.push(newAvatarTrack);
                //allow user to turn mic on
                socket.to(userWhoRequestedAudioAndVideoSlots.id).emit('Media/webrtc/canITurnCameraOn', {
                    type: 'answer',
                    answerValue: true,
                ...
                userWhoRequestedAudioAndVideoSlots.tracks.push(newTrack);
                //allow user to turn mic on
                socket.to(userWhoRequestedAudioAndVideoSlots.id).emit('Media/webrtc/canITurnCameraAndMicOn', {
                    type: 'answer',
                    answerValue: true,
            ...
            socket.emit('Media/webrtc/cancelForceTurnMicOffTimer');
            socket.emit('Media/webrtc/cancelForceTurnCameraOffTimer');
        ...
        function giveAudioSlotToNextUser() {
            ...
            if (socket.webrtcParticipant.micRequestCountdown.queueItem && socket.webrtcParticipant.micRequestCountdown.queueItem.requestType == 'canITurnMicOn') {
                ...
                let allowUserToTurnMicOn = function () {
                    ...
                    //allow user to turn mic on
                    socket.to(giveMicTo.id).emit('Media/webrtc/canITurnMicOn', { type: 'answer', answerValue: true, timeProvided: nextUserInQueue ? socket.webrtcRoom.limits.minimalTimeOfUsingSlot : null, limit: socket.webrtcRoom.limits.audio });
                    ...
                    if (nextUserInQueue) {
                        ...
                        giveMicTo.micRequestCountdown.timer = setTimeout(function () {
                            ...
                            socket.to(giveMicTo.id).emit('Media/webrtc/forceTurnMicOff');
                        }, socket.webrtcRoom.limits.minimalTimeOfUsingSlot);
                ...
                let reserveThisAudioSlotForNextUserInQueue = function (nextItemInMicQueue) {
                    ...
                    userWhoJustGotMicSlot.participant.micRequestCountdown.timer = setTimeout(function () {
                        ...
                        socket.to(userWhoJustGotMicSlot.participant.id).emit('Media/webrtc/forceTurnMicOff');
                    }, socket.webrtcRoom.limits.minimalTimeOfUsingSlot);
                ...
                if (!firstUserInQueue.participant.pendingVideoSlotRequest()) {
                    ...
                    socket.to(firstUserInQueue.participant.id).emit('Media/webrtc/' + firstUserInQueue.requestType, { answerValue: true, limit: socket.webrtcRoom.limits.audio });
                ...
                if (firstUserInQueue.userThatWillGiveSlot) {
                    ...
                    if (nextUserInQueue) {
                        ...
                        socket.to(firstUserInQueue.userThatWillGiveSlot.id).emit('Media/webrtc/cancelForceTurnMicOffTimer');
        ...
        function recalculateAllMicTimers(timeDiff) {
            for (let q in socket.webrtcRoom.micRequestsQueue) {
                ...
                socket.to(socket.webrtcRoom.id).emit('Media/webrtc/requestMic', emitMsg);
            }
        }
    }
