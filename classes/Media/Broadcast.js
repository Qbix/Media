...
/**
 * Start internal listener for Media Broadcast
 * @method listen
 * @static
    ...
    broadcastNamespace.on('connection', function(socket) {
        ...
        function findReceiver(donorParticipant) {
            ...
            for(let p in broadcastRoom.participants) {
                ...
                if(broadcastRoom.participants[p].role == 'receiver' && broadcastRoom.participants[p].donors.length == 0) {
                    ...
                    broadcastNamespace.to(nspName + '#' + donorParticipant.id).emit('Media/broadcast/participantConnected', {
            ...
            for(let p in broadcastRoom.participants) {
                ...
                if(((roomParticipant.role == 'receiver' && roomParticipant.donors.length != 0) || roomParticipant.role == 'publisher')
                    ...
                    askPermissionToConnect(roomParticipant).then(function (answer) {
                        ...
                        /*broadcastNamespace.to(nspName + '#' + roomParticipant.id).emit('Media/broadcast/participantConnected', {
        ...
        async function askPermissionToConnect(roomParticipant) {
            await new Promise(resolve => {
                broadcastNamespace.to(nspName + '#' + roomParticipant.id).emit('Media/broadcast/canIConnect', {}, (answer) => {
                    resolve(answer);
                });
            });
        }
        ...
        socket.on('Media/broadcast/confirmOnlineStatus', function(message) {
            ...
            socket.to(nspName + '#' + message.targetSid).emit('Media/broadcast/confirmOnlineStatus', message);
        ...
        socket.on('Media/broadcast/canISendOffer', function(message) {
            ...
            message.fromSid = socket.client.id;
            socket.to(nspName + '#' + message.targetSid).emit('Media/broadcast/canISendOffer', message);
        ...
        socket.on('Media/broadcast/signalling', function(message) {
            ...
            socket.to(nspName + '#' + message.targetSid).emit('Media/broadcast/signalling', message);
        });
        ...
        socket.on('disconnect', function() {
            ...
            socket.broadcast.to(roomId).emit('Media/broadcast/participantDisconnected', socket.client.id);
        });
