(function (Q, $, window, undefined) {
    ...
    /**
     * Renders an interface to handle Media/calls
     * @class Media calls
     * @constructor
     * @param {Object} [options] any options for the tool
     * @param {Q.Event} [options.maxCalls=0] Max calls can be related to this category
     * @param {Q.Event} [options.publisherId=Users.currentCommunityId] Publisher of Media/calls/main stream
     * @param {Q.Event} [options.streamName=Media/calls/main] Category stream name
     */
    Q.Tool.define("Media/calls", function(options) {
            ...
            Streams.get.force(state.publisherId, state.streamName, function (err) {
                ...
                var calls = Q.getObject(["relatedToTotals", state.relationType, "Media/webrtc"], this) || 0;
                var maxCalls = tool.getMaxCalls();
                tool.available(calls < maxCalls);
                this.onMessage("Streams/relation/available").set(function (message) {
                    tool.available(true);
                }, tool);
                this.onMessage("Streams/relation/unavailable").set(function (message) {
            ...
            Q.addStylesheet('{{Media}}/css/tools/calls.css', { slotName: 'Streams' }, pipe.fill("style"));
        },
...

        {
            ...
            initMainRoom: function() {
                ...
                var WebRTCClientUI = tool.state.mainWebrtcRoom = Media.WebRTC.start({
                    ...
                    streamName: tool.state.eventsStream.fields.name, /*"Media/webrtc/live"*/
            ...
            call: function () {
                ...
                $toolElement.addClass("Streams_calls_call").on(Q.Pointer.fastclick, function () {
                    Q.prompt(null, function (content) {
                        ...
                        //start webrtc waiting room and relate it to Media/calls/main so hosts can see new call
    ...
    Q.Template.set("Media/calls/settings",
